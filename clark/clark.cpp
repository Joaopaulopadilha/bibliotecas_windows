...
// =============================================================================
// JS DA BARRA DE TITULO CUSTOMIZADA
// =============================================================================
static const char* CLARK_TITLEBAR_JS = R"JS(
(function() {
    function __clark_injetar_barra() {
    ...
    if (document.body) {
        __clark_injetar_barra();
    } else {
        document.addEventListener('DOMContentLoaded', __clark_injetar_barra);
...
// Registra o JS da barra via init() para rodar apos cada navegacao
static void clark_registrar_titlebar_init(Janela* jan) {
    if (jan->wv) {
        jan->wv->init(CLARK_TITLEBAR_JS);
    }
}
    ...
    while (!cmds.empty()) {
        ...
        switch (cmd.tipo) {
            case Janela::Comando::CMD_HTML:
                if (jan->wv) {
                    jan->wv->set_html(cmd.arg1);
                    // Se sem_borda esta ativo, re-registra o init para a nova pagina
                    if (jan->sem_borda) {
                        clark_registrar_titlebar_init(jan);
                    }
                }
                break;
            ...
            case Janela::Comando::CMD_SEM_BORDA: {
                if (jan->wv) {
                    ...
                    if (hwnd) {
                        ...
                        // Registra via init() para rodar apos cada navegacao
                        clark_registrar_titlebar_init(jan);
                        // Tambem injeta agora na pagina atual
                        jan->wv->eval(CLARK_TITLEBAR_JS);
                    }
#else
                    // =========================================================
